...
if (-not [string]::IsNullOrWhiteSpace($ScriptPath)) {
    ...
    $hookContent += ('pwsh -NoProfile -ExecutionPolicy Bypass -File "{0}" -RepoRoot "$REPO_ROOT" || true' -f $normalizedScriptPath)

    $hookContent += 'pwsh -NoProfile -ExecutionPolicy Bypass -File "$REPO_ROOT/scripts/obsidian/sync-docs.ps1"'

} else {

    $hookContent += 'pwsh -NoProfile -ExecutionPolicy Bypass -File "$REPO_ROOT/scripts/obsidian/write-commit-journal.ps1" -RepoRoot "$REPO_ROOT" || true'
