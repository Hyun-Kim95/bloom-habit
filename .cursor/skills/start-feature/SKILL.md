...
## 절차
1. `.cursor/rules/60-delivery-gates.mdc` Gate 1을 점검한다. 이 레포에서는 `docs/README.md`와 **`docs/spec/09-prd-vs-current-gap.md`**, **`docs/spec/10-prd-todo-list.md`**가 범위·갭·우선 TODO의 기본 산출이며, API/ERD 변경이 있으면 **`docs/spec/05-erd-api-spec.md`**(및 필요 시 `docs/spec/07-admin-api.md` 등)를 코딩 전에 맞춘다. 미충족이면 구현을 시작하지 않고 `plan-feature` 또는 `prd-agent`로 돌아간다.
2. 요청을 기능 단위로 분해한다.
