...
class FlutterNaverLoginPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private fun login(result: Result) {
        ...
        // HabitFable: 로그인 전 logout()을 호출하지 않음.
        // 동의 완료 후 프로필 조회만 실패한 경우 토큰이 남는데, logout() 시 매 시도마다
        // 제3자 동의 화면이 다시 뜨는 문제가 있음. 세션 정리는 앱(Dart)에서만 수행.
    ...
    fun jsonObjectToMap(jObject: JSONObject): HashMap<String, String> {
        ...
        while (keys.hasNext()) {
            ...
            if (jObject.isNull(key)) continue
            map[key] = when (val raw = jObject.get(key)) {
                is String -> raw
                else -> raw.toString()
            }
        }
