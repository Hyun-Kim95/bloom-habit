...
**이 레포 문서 전제:** `docs/README.md`를 허브로 하며, 범위·갭·우선 TODO·API/ERD 초안은 **`docs/spec/`** 아래 `09-prd-vs-current-gap.md`, `10-prd-todo-list.md`, `05-erd-api-spec.md` 등으로 관리한다. 이들을 갱신·합의한 것은 신규 기능에 대해 Gate 1이 요구하는 “동등 산출”에 해당할 수 있다(세부는 `.cursor/rules/60-delivery-gates.mdc`). **일상 유지보수·소규모 변경**은 해당 파일의 적용 범위·예외와 본 파일의 **직접 처리 가능한 예외**를 따른다.
